  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
        <div>
          <h2 style={{ fontSize: '1.5rem', marginBottom: '0.5rem' }}>Teachers</h2>
          <p style={{ color: 'var(--color-text-muted)' }}>Manage teachers in the system.</p>
        </div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={3} style={{ padding: '2rem', textAlign: 'center', color: 'var(--color-text-muted)' }}>No teachers found.</td></tr>
              ) : (
                teachers.map(teacher => (
                  <tr key={teacher.id} style={{ borderBottom: '1px solid var(--color-border)' }}>
                    ...
                    <td style={{ padding: '1rem' }}>{teacher.user?.email || 'N/A'}</td>
                  </tr>
                ))
              )}
            </tbody>
